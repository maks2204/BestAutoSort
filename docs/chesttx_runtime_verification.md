...
| # | Claim | Pinned by |
|---|-------|-----------|
| C1 | Trust-gate seam confirmed (ClassifySenderBinding/SpoofTerminal, ephemeral spoof, no victim mutation); production OnTxRequest spoof-branch wiring assumed (shares seam by statement, unexecuted in harness) | SPOOF_VictimTxIdNotBurned, SPOOF_IncompatiblePeerCannotRaiseVictimFloor, SPOOF_CachedResultNotLeaked, SPOOF_SenderBindingSeam, FENCE_SpoofNeverFences, SENDER0_LegacyFreshAndQueryDocumented |
| C2 | Quarantine-terminal seam confirmed (remote/local/queued Indeterminate, fence-first, no-resurrect matrix); production remote/local/queued bodies assumed (Unity-coupled, unexecuted — same caveat as C14) | QUARANTINE_Add, QUARANTINE_TakeDelayedDuplicateAfterRecovery, QUARANTINE_QueuedLocalRemoteShareTerminal, QUARANTINE_HandoffDropPersistenceFailure, QUARANTINE_FreshAddTerminalNeverExecutesAfterRecovery, QUARANTINE_TakeTerminalNeverExecutesAfterRecovery, QUARANTINE_QueuedFreshTxNeverResurrects |
| C3 | Pending-drain seam confirmed (Decide matrix); production PumpPending closures assumed (FakePending harness; in-loop mutation/destroyed-finalize/reentrancy unexecuted) | PENDING_DecideMatrix, PENDING_MultipleTimeoutsOnePump, PENDING_TerminalCallbackAndClaimsExactlyOnce |
| C4 | Claim-lease primitive + mirror shape confirmed; production Submit try/finally wiring assumed (synthetic harness modes, live ItemData refs unexecuted) | CLAIM_DuplicatePruned, CLAIM_PrePendingFailuresRelease, CLAIM_TerminalReleaseExactlyOnce |
| C5 | Drag gate/arithmetic seam confirmed; production RestoreDragRemainder preservation assumed (already-removed path unexecuted) | DRAG_Rejected, DRAG_Partial, DRAG_Indeterminate |
| C6 | Fence-first seam order + crash windows confirmed on the model; production ApplyJob/TryPersistExecutionFence/CommitResult ZDO-key order assumed (same caveat as C14) | TxFenceCrashTests group |
...
| C15 | Transient seams confirmed (RAM-only map both-fail only, flagged-refusal terminal, flagged Query-before-cache-miss, DecideTransient/DecideTransientSend/CanSendStoredMutationPayload, ExecuteCalls==0 never-executes, restart-from-bytes); production OnTxRequest/RespondQuery flagged legs and SendTransientRetry/PumpPending transient branch assumed (mirror statement-by-statement, build+review covered) | TxTransientTests + TxTransientSendTests groups (core pins; production send/pump/request wiring assumed, observed via O-table send/retain logs) |
| C16 | Source-clamp helper + mirror order confirmed (ClampAddAmount/CountsAsFull, depleted-skip, full-counting); production ExecuteAdd clamp-before-credit wiring assumed (test-local ManagerCredit mirror, reverting the production block keeps suite green — verified by review) | TxSourceClampTests group (helper + mirror pins; production ExecuteAdd order assumed) |
